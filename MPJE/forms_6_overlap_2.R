...
for(i in states){
  ...
  assign(paste(i, "competencies", sep="_"), competencies)
...
competency_tables <- lapply(ls(pattern="??_competencies"), function(x) get(x))
blueprint_by_competency<-do.call(rbind, competency_tables)
write.csv(blueprint_by_competency, "blueprint_by_competency.csv", row.names=FALSE)
