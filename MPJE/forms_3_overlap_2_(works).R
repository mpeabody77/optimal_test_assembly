...
for(i in states){
  ...
  n_forms <- 3 #Number of test forms (F)
  ...
  for (k in 1:n_items){
    add.constraint(lprec,rep(1,2,3),"<=",2,indices=c(k,
                                                   n_items+k, 
                                                   2*n_items+k))
  }
  ...
  y = matrix(0,length(x),3) #Define empty matrix with item information function values along theta axis for all items
