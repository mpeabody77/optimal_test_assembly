...
for (k in 1:n_items){
  add.constraint(lprec,rep(1,2),"<=",1,indices=c(k,n_items+k))
}
...
y = matrix(0,length(x),2) #Define empty matrix with item information function values along theta axis for all items
...
plot(x,y[,1], type="l", lty=1, xlab='Theta',ylab='Information')
lines(x,y[,2], type="l", lty=1, xlab='Theta',ylab='Information')
title(i)
dev.off()
...
blueprint.table<-aux %>%
  group_by(Content) %>%
  summarize(A=sum(FORM_A),B=sum(FORM_B)) %>%
  mutate(State = i)
...
list_tables <- lapply(ls(pattern="?? table"), function(x) get(x))
